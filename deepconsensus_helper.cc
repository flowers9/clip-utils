...
int main(const int argc, const char ** const argv) {
	if (argc != 3 || !*argv[1] || !*argv[2]) {
		std::cerr << "usage: deepconsensus_help <aligned.subreads.bam> <subreads_to_ccs.bam>\n";
		return 1;
	}
	...
	while (f_in.GetNext(r)) {
		...
		// (r.HoleNumber()), but not the reference, so probably best
		...
		// only print alignments that match subread to same ccs read
