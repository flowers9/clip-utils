...
static bool opt_cleanup, opt_indel, opt_report;
static double opt_overlap_cutoff;
static int fd_log, opt_change, opt_confirm, opt_length_cutoff;
...
struct Match {
	...
	void clean(void);
    public:
	long length, start, stop, score, s_start, s_stop;
	std::string qs, ss, tag, id;
...
class BlatFile {
	...
	bool _is_current;
	int _fd;
	std::string _filename, _current_id;
    public:
...
class FastaFile {
	...
	std::string _header, _id, _seq, _next_entry;
...
void Match::clean() {
	...
	std::string::size_type i = tag.find(' ');
	while (i != std::string::npos) {
		if (good.find(ss[i]) == std::string::npos) {
			tag[i] = '|';
		}
		i = tag.find(' ', i + 1);
	}
...
// returns if match should be used; cleans match if so
...
// turn homopolymers surround k into n's
...
bool BlatFile::find_next(const std::string &id) {
	...
	if (_current_id.empty()) {
		while (pfgets(_fd, _current_id) != -1 && _current_id.compare(0, 6, "Query=") != 0) { }
		if (_current_id.empty()) {
			close_blat();
			return 0;
		}
		// id is $1: ^Query=\s*(\S+)
		std::string::size_type i = 6;
		_current_id = strtostr(_current_id, &i);
	}
...
// XXX - this doesn't do much/any error checking of the file contents
bool BlatFile::read_next(size_t seq_length, Match &b) {
	for (;;) {		// loop over all current matches
		...
		for (;;) {	// loop until next good match
			while (pfgets(_fd, line) != -1) {
				if (line[0] == '>') {
					...
					pfgets(_fd, line);
					i = line.find("Length = ");
					b.length = atoi(line.substr(i + 9).c_str());
					pfgets(_fd, line);	// blank
					pfgets(_fd, line);	// score
					break;
				} else if (line.compare(0, 6, " Score") == 0) {
					break;
			...
			b.score = atoi(line.substr(line.find("Score ") + 8).c_str());
			pfgets(_fd, line);
			identity = atoi(line.substr(line.find("Identities = ") + 13).c_str());
			pfgets(_fd, line);	// strand
			pfgets(_fd, line);	// blank
		...
		pfgets(_fd, line);
		// parse tag line
		b.tag = line.substr(header_length);
		pfgets(_fd, line);
		// parse ^Sbjct:\s+(\d+)\s+(\S+)\s+(\d+)$
...
void FastaFile::read_next_qual() {
	...
	std::string line;
	if (pfgets(_fd_qual, line) == -1) {
		return;
	} else if (line[0] == '>') {	// first line in file
		pfgets(_fd_qual, line);
	}
	while (line[0] != '>') {
	...
	if (_qual.size() != 0 && _qual.size() != _seq.size()) {
		fprintf(stderr, "Warning: lengths mismatch between sequence and qual: %lu != %lu: %s\n", _seq.size(), _qual.size(), _id.c_str());
	}
}

bool FastaFile::read_next() {
	if (_next_entry.empty()) {
	...
	if (!_header.empty()) {
		...
		_id = strtostr(_header, &i);
		_header += "\n";
		while (pfgets(_fd_fasta, _next_entry) != -1 && _next_entry[0] != '>') {
			_seq += _next_entry;
		}
		read_next_qual();
	}
...
void FastaFile::print(int fd_fasta_out, int fd_qual_out) const {
	...
	if (fd_qual_out != -1) {
		...
		if (!_qual.empty() && pfputs(fd_qual_out, itoa(_qual[i]) + "\n") == -1) {
			exit(1);
		}
	}
}
...
static std::string find_qual_file(const std::string &t, bool skip_check) {
	std::string filename(t);	// so I can modify it
	std::string suffix;
	...
	filename += suffix;
	if (skip_check || stat(filename.c_str(), &buf) == 0) {
		return filename;
	}
	return "";
}
...
static void get_opts(int argc, char **argv) {
	...
	opt_report = 0;
	int c;
	while ((c = getopt(argc, argv, "cIk:l:m:n:rt:Vx:")) != EOF) {
		switch (c) {
		    case 'c':
			opt_cleanup = 1;
			break;
...
static void open_blats(const std::string &tmp_dir, const std::string &index, std::list<BlatFile> &blats) {
	...
	while ((a = readdir(dir)) != NULL) {
		...
		// pattern match for ^b$n\.\d+\.bz2$, where $n == index
		if (s.size() >= n + 7 && s[0] == 'b' && s.compare(1, n, index) == 0 && s[n + 1] == '.' && s.compare(s.size() - 4, 4, ".bz2") == 0 && s.find_first_not_of("0123456789", n + 2) == s.size() - 4) {
...
static void get_matches(std::list<BlatFile> &blats, size_t seq_length, std::list<Match> &matches, std::list<Match> &all_matches) {
	std::list<BlatFile>::iterator a = blats.begin();
	std::list<BlatFile>::iterator end_a = blats.end();
	for (; a != end_a; ++a) {
		if (!a->is_current()) {
...
static void make_changes(std::list<Match> &good_matches, std::list<Match> &bad_matches, std::vector<int> &confirm, std::string &confirms, std::map<long, std::map<char, int> > &changes, FastaFile &fasta, std::map<long, char> &changes_made, const std::vector<int> &unpad) {
	for (;;) {
...
static void process_blats(FastaFile &fasta, std::list<BlatFile> &blats, int fd_fasta, int fd_qual) {
	...
	get_matches(blats, fasta.seq().size(), matches, all_matches);
	...
	make_changes(good_matches, bad_matches, confirm, confirms, changes, fasta, changes_made, unpad);
	if (opt_report) {
